...
const categoryColors: Record<string, string> = {
  ...
  "AI & Agents": "bg-violet-50 text-violet-600",
};

const fallbackStyles: Record<string, { gradient: string; icon: React.ElementType }> = {
  "AI Intelligence Agent": { gradient: "from-violet-600 via-purple-600 to-indigo-700", icon: FiCpu },
  "Inventory Enterprise": { gradient: "from-blue-600 via-blue-700 to-indigo-700", icon: FiDatabase },
};
    ...
    <section id="projects" className="py-16 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {projects.slice(0, 6).map((project, i) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-violet-200 hover:shadow-xl hover:shadow-violet-100/30 transition-all group"
            >
              {/* Image */}
              {project.image ? (
                <div className="h-40 relative overflow-hidden bg-gradient-to-br from-violet-500 to-violet-700">
                  <Image
                    src={project.image}
                    alt={project.title}
                    fill
                ...
                <div className={`h-40 relative overflow-hidden bg-gradient-to-br ${fallbackStyles[project.title]?.gradient || "from-violet-500 to-violet-700"}`}>
              ...
              <div className="p-5">
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${categoryColors[project.category] || "bg-violet-50 text-violet-600"}`}>
                  {project.category}
                </span>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {project.tech.map((t, j) => (
                    <span key={j} className="text-[10px] px-2 py-1 bg-gray-50 text-gray-500 rounded-md border border-gray-100 font-medium">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-2">
                  <a
                    ...
                    className="w-8 h-8 rounded-lg bg-gray-50 hover:bg-violet-50 flex items-center justify-center text-gray-400 hover:text-violet-600 transition-all"
                  >
                    <FiGithub size={14} />
                  </a>
                  {project.live !== "#" && (
        ...
        >
          <Link
            href="/projects"
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-violet-600 text-white text-sm font-medium rounded-xl hover:bg-violet-700 transition-all hover:shadow-lg hover:shadow-violet-200"
          >
