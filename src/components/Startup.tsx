      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-gray-900">
            Building <span className="text-violet-600">Marsa Empower</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-6 items-start">
          ...
          >
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className="inline-flex items-center gap-1 bg-violet-100 text-violet-700 px-2.5 py-1 rounded-full text-xs font-medium">
            ...
            <p className="text-xs text-violet-600 font-semibold uppercase tracking-wider mb-1">Founder & CEO</p>
            <h3 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight mb-3">
              Empowering individuals, ideas & <span className="text-violet-600">communities</span>
            </h3>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {responsibilities.map(({ icon: Icon, text }, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 bg-gray-50 border border-gray-100 rounded-lg px-3 py-1.5 text-xs text-gray-600 hover:border-violet-200 hover:bg-violet-50 transition-colors">
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white font-semibold rounded-xl hover:bg-violet-700 transition-all hover:shadow-lg hover:shadow-violet-200 text-sm"
            >
          ...
          >
            {/* Logo Card */}
            <div className="bg-gradient-to-br from-violet-600 via-violet-700 to-violet-900 rounded-2xl p-5 text-white relative overflow-hidden">
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-violet-200 text-xs">Digital Empowerment Platform</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
              <h4 className="text-xs font-semibold text-gray-900 uppercase tracking-wider mb-3">Company Details</h4>
              <div className="space-y-2.5">
                {companyDetails.map(({ icon: Icon, label, value, href, external }, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center border border-gray-200 flex-shrink-0">
                      <Icon size={14} className="text-violet-600" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[10px] text-gray-400 leading-none mb-0.5">{label}</p>
                      {href ? (
                        <a href={href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} className="text-xs font-medium text-gray-700 hover:text-violet-600 transition-colors flex items-center gap-1">
                          {value} {external && <FiExternalLink size={10} className="text-gray-400" />}
                        </a>
                      ) : (
